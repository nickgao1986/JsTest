...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'table', ELEM = '.layer-table', THIS = 'layer-this', SHOW = 'layer-show', HIDE = 'layer-hide', DISABLED = 'layer-disabled', NONE = 'layer-none'
  
  ,ELEM_VIEW = 'layer-table-view', ELEM_TOOL = '.layer-table-tool', ELEM_BOX = '.layer-table-box', ELEM_INIT = '.layer-table-init', ELEM_HEADER = '.layer-table-header', ELEM_BODY = '.layer-table-body', ELEM_MAIN = '.layer-table-main', ELEM_FIXED = '.layer-table-fixed', ELEM_FIXL = '.layer-table-fixed-l', ELEM_FIXR = '.layer-table-fixed-r', ELEM_TOTAL = '.layer-table-total', ELEM_PAGE = '.layer-table-page', ELEM_SORT = '.layer-table-sort', ELEM_EDIT = 'layer-table-edit', ELEM_HOVER = 'layer-table-hover'
  ...
  ,TPL_HEADER = function(options){
    ...
    options = options || {};
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layer-table" '
      ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
      ,'<thead>'
      ,'{{# layer.each(d.data.cols, function(i1, item1){ }}'
        ,'<tr>'
        ,'{{# layer.each(item1, function(i2, item2){ }}'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layer-hide{{# } }}{{# if(isSort){ }} layer-unselect{{# } }}{{# if(!item2.field){ }} layer-table-col-special{{# } }}">'
            ,'<div class="layer-table-cell laytable-cell-'
              ,'{{# if(item2.colGroup){ }}'
                ,'group'
              ,'{{# } else { }}'
                ,'{{d.index}}-{{i1}}-{{i2}}'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(isSort){ }}'
                  ,'<span class="layer-table-sort layer-inline"><i class="layer-edge layer-table-sort-asc" title="升序"></i><i class="layer-edge layer-table-sort-desc" title="降序"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
  ...
  //tbody区域模板
  ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layer-table" '
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
  
  //主模板
  ,TPL_MAIN = ['<div class="layer-form layer-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'

    ,'{{# if(d.data.toolbar){ }}'
    ,'<div class="layer-table-tool">'
      ,'<div class="layer-table-tool-temp"></div>'
      ,'<div class="layer-table-tool-self"></div>'
    ,'</div>'
    ,'{{# } }}'
    
    ,'<div class="layer-table-box">'
      ,'{{# if(d.data.loading){ }}'
      ,'<div class="layer-table-init" style="background-color: #fff;">'
        ,'<i class="layer-icon layer-icon-loading layer-anim layer-anim-rotate layer-anim-loop"></i>'
      ,'</div>'
      ,'{{# } }}'
      ...
      ,'<div class="layer-table-header">'
        ,TPL_HEADER()
      ,'</div>'
      ,'<div class="layer-table-body layer-table-main">'
        ,TPL_BODY
      ,'</div>'
      
      ,'{{# if(left){ }}'
      ,'<div class="layer-table-fixed layer-table-fixed-l">'
        ,'<div class="layer-table-header">'
          ,TPL_HEADER({fixed: true}) 
        ,'</div>'
        ,'<div class="layer-table-body">'
          ,TPL_BODY
        ,'</div>'      
      ,'</div>'
      ,'{{# }; }}'
      ...
      ,'{{# if(right){ }}'
      ,'<div class="layer-table-fixed layer-table-fixed-r">'
        ,'<div class="layer-table-header">'
          ,TPL_HEADER({fixed: 'right'})
          ,'<div class="layer-table-mend"></div>'
        ,'</div>'
        ,'<div class="layer-table-body">'
          ,TPL_BODY
        ,'</div>'
    ...
    ,'{{# if(d.data.totalRow){ }}'
      ,'<div class="layer-table-total">'
        ,'<table cellspacing="0" cellpadding="0" border="0" class="layer-table" '
        ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
          ,'<tbody><tr><td><div class="layer-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>'
      , '</table>'
      ,'</div>'
    ,'{{# } }}'
    ...
    ,'<div class="layer-table-page">'
      ,'<div id="layer-table-page{{d.index}}"></div>'
    ,'</div>'
    ,'{{# } }}'
  ...
  Class.prototype.renderToolbar = function(){
    ...
    //添加工具栏左侧模板
    var leftDefaultTemp = [
      '<div class="layer-inline" lay-event="add"><i class="layer-icon layer-icon-add-1"></i></div>'
      ,'<div class="layer-inline" lay-event="update"><i class="layer-icon layer-icon-edit"></i></div>'
      ,'<div class="layer-inline" lay-event="delete"><i class="layer-icon layer-icon-delete"></i></div>'
    ].join('')
    ,elemToolTemp = that.layTool.find('.layer-table-tool-temp');
    ...
    var layout = {
      filter: {
        ...
        ,icon: 'layer-icon-cols'
      }
      ,exports: {
        title: '导出'
        ,layEvent: 'LAYTABLE_EXPORT'
        ,icon: 'layer-icon-export'
      }
      ,print: {
        title: '打印'
        ,layEvent: 'LAYTABLE_PRINT'
        ,icon: 'layer-icon-print'
      }
    }, iconElem = [];
    ...
    if(typeof options.defaultToolbar === 'object'){
      layui.each(options.defaultToolbar, function(i, item){
        ...
        if(thisItem){
          iconElem.push('<div class="layer-inline" title="'+ thisItem.title +'" lay-event="'+ thisItem.layEvent +'">'
            +'<i class="layer-icon '+ thisItem.icon +'"></i>'
          +'</div>');
        }
      });
    }
    that.layTool.find('.layer-table-tool-self').html(iconElem.join(''));
  }
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            ...
            if(!item3.field) classNames.push('layer-table-col-special'); //插入特殊列样式
            return classNames.join(' ');
          }() +'">'
            ,'<div class="layer-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
    ...
    if(options.page){
      options.page = $.extend({
        elem: 'layer-table-page' + options.index
        ,count: count
        ,limit: options.limit
        ,limits: options.limits || [10,20,30,40,50,60,70,80,90]
        ,groups: 3
        ,layout: ['prev', 'page', 'next', 'skip', 'count', 'limit']
        ,prev: '<i class="layer-icon">&#xe603;</i>'
        ,next: '<i class="layer-icon">&#xe602;</i>'
  ...
  Class.prototype.loading = function(hide){
    ...
    if(options.loading){
      if(hide){
        ...
        that.layInit = $(['<div class="layer-table-init">'
          ,'<i class="layer-icon layer-icon-loading layer-anim layer-anim-rotate layer-anim-loop"></i>'
        ,'</div>'].join(''));
  ...
  Class.prototype.scrollPatch = function(){
    ...
    ,addPatch = function(elem){
      if(scollWidth && scollHeight){
        elem = elem.eq(0);
        if(!elem.find('.layer-table-patch')[0]){
          var patchElem = $('<th class="layer-table-patch"><div class="layer-table-cell"></div></th>'); //补丁元素
        ...
        elem.find('.layer-table-patch').remove();
      }
    }
  ...
  Class.prototype.events = function(){
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      ,openPanel = function(sets){
        var list = $(sets.list)
        ,panel = $('<ul class="layer-table-tool-panel"></ul>');
        ...
        //插入元素
        othis.find('.layer-table-tool-panel')[0] || othis.append(panel);
        that.renderForm();
      ...
      switch(events){
        ...
        case 'LAYTABLE_PRINT': //打印
          ...
          html.find('th.layer-table-patch').remove(); //移除补丁
          html.find('.layer-table-col-special').remove(); //移除特殊列
    ...
    }).find(ELEM_SORT+' .layer-edge ').on('click', function(e){
      var othis = $(this)
      ,index = othis.index()
    ...
    that.layBody.on('click', 'td', function(e){
      ...
      //显示编辑表单
      if(editType){
        var input = $('<input class="layer-input '+ ELEM_EDIT +'">');
    ...
    //单元格展开图标
    var ELEM_GRID = 'layer-table-grid', ELEM_GRID_DOWN = 'layer-table-grid-down', ELEM_GRID_PANEL = 'layer-table-grid-panel'
    ,gridExpand = function(hide){
      ...
      if(hide){
        othis.find('.layer-table-grid-down').remove();
      } else if(elemCell.prop('scrollWidth') > elemCell.outerWidth()){
        if(elemCell.find('.'+ ELEM_GRID_DOWN)[0]) return;
        othis.append('<div class="'+ ELEM_GRID_DOWN +'"><i class="layer-icon layer-icon-down"></i></div>');
      }
    };
    ...
    that.layBody.on('click', '.'+ ELEM_GRID_DOWN, function(e){
      ...
      that.tipsIndex = layer.tips([
        '<div class="layer-table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
          if(options.size === 'sm'){
          ...
          ,elemCell.html()
        ,'</div>'
        ,'<i class="layer-icon layer-table-tips-c layer-icon-close"></i>'
      ].join(''), elemCell[0], {
        tips: [3, '']
        ,time: -1
        ,anim: -1
        ...
        ,isOutAnim: false
        ,skin: 'layer-table-tips'
        ,success: function(layero, index){
          layero.find('.layer-table-tips-c').on('click', function(){
            layer.close(index);
          });
        }
      });
