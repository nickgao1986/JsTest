...
layui.define('jquery', function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'slider', DISABLED = 'layer-disabled', ELEM_VIEW = 'layer-slider', SLIDER_BAR = 'layer-slider-bar', SLIDER_WRAP = 'layer-slider-wrap', SLIDER_WRAP_BTN = 'layer-slider-wrap-btn', SLIDER_TIPS = 'layer-slider-tips', SLIDER_INPUT = 'layer-slider-input', SLIDER_INPUT_TXT = 'layer-slider-input-txt', SLIDER_INPUT_BTN = 'layer-slider-input-btn', ELEM_HOVER = 'layer-slider-hover'
  ...
  Class.prototype.render = function(){
    ...
    //滑块
    var temp = '<div class="layer-slider '+ (options.type === 'vertical' ? 'layer-slider-vertical' : '') +'">'+ (options.tips ? '<div class="layer-slider-tips"></div>' : '') +
    '<div class="layer-slider-bar" style="background:'+ theme +'; '+ (options.type === 'vertical' ? 'height' : 'width') +':'+ scale +';'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || 0) +';"></div><div class="layer-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ (scaleFir || scale) +';">' +
    '<div class="layer-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>'+ (options.range ? '<div class="layer-slider-wrap" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ scaleSec +';"><div class="layer-slider-wrap-btn" style="border: 2px solid '+ theme +';"></div></div>' : '') +'</div>';
    ...
    if(options.showstep){
      ...
      for(var i = 1; i < number + 1; i++) {
        ...
        if(step < 100){
          item += '<div class="layer-slider-step" style="'+ (options.type === 'vertical' ? 'bottom' : 'left') +':'+ step +'%"></div>'
        }
      };
    ...
    if(options.input && !options.range){
      var elemInput = $('<div class="layer-slider-input layer-input"><div class="layer-slider-input-txt"><input type="text" class="layer-input"></div><div class="layer-slider-input-btn"><i class="layer-icon layer-icon-up"></i><i class="layer-icon layer-icon-down"></i></div></div>');
  ...
  Class.prototype.slide = function(setValue, value, i){
    ...
    //拖拽元素
    ,elemMove = $(['<div class="layer-auxiliar-moving" id="LAY-slider-moving"></div'].join(''))
