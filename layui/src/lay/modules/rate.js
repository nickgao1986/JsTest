...
layui.define('jquery',function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'rate',ELEM_VIEW = 'layer-rate', ICON_RATE = 'layer-icon-rate', ICON_RATE_SOLID = 'layer-icon-rate-solid', ICON_RATE_HALF = 'layer-icon-rate-half'
  
  ,ICON_SOLID_HALF = 'layer-icon-rate-solid layer-icon-rate-half',  ICON_SOLID_RATE = 'layer-icon-rate-solid layer-icon-rate',  ICON_HALF_RATE = 'layer-icon-rate layer-icon-rate-half'
  ...
  Class.prototype.render = function(){
    ...
    //组件模板
    var temp = '<ul class="layer-rate" '+ (options.readonly ? 'readonly' : '') +'>';
    for(var i = 1;i <= options.length;i++){
      var item = '<li class="layer-inline"><i class="layer-icon '
      ...
      if(options.half){
        if(parseInt(options.value) !== options.value){
          if(i == Math.ceil(options.value)){
            temp = temp + '<li><i class="layer-icon layer-icon-rate-half" '+ style +'></i></li>';
          }else{
            temp = temp + item 
          } 
        }else{
    ...
    temp += '</ul>' + (options.text ? ('<span class="layer-inline">'+ options.value + '星') : '') + '</span>';
