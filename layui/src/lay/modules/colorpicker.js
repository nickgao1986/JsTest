...
layui.define('jquery', function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'colorpicker', SHOW = 'layer-show', THIS = 'layer-this', ELEM = 'layer-colorpicker'
  
  ,ELEM_MAIN = '.layer-colorpicker-main', ICON_PICKER_DOWN = 'layer-icon-down', ICON_PICKER_CLOSE = 'layer-icon-close'
  ,PICKER_TRIG_SPAN = 'layer-colorpicker-trigger-span', PICKER_TRIG_I = 'layer-colorpicker-trigger-i', PICKER_SIDE = 'layer-colorpicker-side', PICKER_SIDE_SLIDER = 'layer-colorpicker-side-slider'
  ,PICKER_BASIS = 'layer-colorpicker-basis', PICKER_ALPHA_BG = 'layer-colorpicker-alpha-bgcolor', PICKER_ALPHA_SLIDER = 'layer-colorpicker-alpha-slider', PICKER_BASIS_CUR = 'layer-colorpicker-basis-cursor', PICKER_INPUT = 'layer-colorpicker-main-input'
  ...
  Class.prototype.render = function(){
    ...
    //颜色选择框对象
    ,elemColorBox = $(['<div class="layer-unselect layer-colorpicker">'
      ,'<span '+ (options.format == 'rgb' && options.alpha
          ? 'class="layer-colorpicker-trigger-bgcolor"'
        : '') +'>'
        ,'<span class="layer-colorpicker-trigger-span" '
          ...
          ,'<i class="layer-icon layer-colorpicker-trigger-i '+ (options.color
            ? ICON_PICKER_DOWN 
          : ICON_PICKER_CLOSE) +'"></i>'
        ,'</span>'
      ,'</span>'
    ...
    options.size && elemColorBox.addClass('layer-colorpicker-'+ options.size); //初始化颜色选择框尺寸
    
    //插入颜色选择框
    othis.addClass('layer-inline').html(
  ...
  Class.prototype.renderPicker = function(){
    ...
    //颜色选择器对象
    ,elemPicker = that.elemPicker = $(['<div id="layer-colorpicker'+ that.index +'" data-index="'+ that.index +'" class="layer-anim layer-anim-upbit layer-colorpicker-main">'
      //颜色面板
      ,'<div class="layer-colorpicker-main-wrapper">'
        ,'<div class="layer-colorpicker-basis">'
          ,'<div class="layer-colorpicker-basis-white"></div>'
          ,'<div class="layer-colorpicker-basis-black"></div>'
          ,'<div class="layer-colorpicker-basis-cursor"></div>'
        ,'</div>'
        ,'<div class="layer-colorpicker-side">'
          ,'<div class="layer-colorpicker-side-slider"></div>'
        ,'</div>'
      ,'</div>'
      
      //透明度条块
      ,'<div class="layer-colorpicker-main-alpha '+ (options.alpha ? SHOW : '') +'">'
        ,'<div class="layer-colorpicker-alpha-bgcolor">'
          ,'<div class="layer-colorpicker-alpha-slider"></div>'
        ,'</div>'
      ,'</div>'
      ...
      ,function(){
        if(options.predefine){
          var list = ['<div class="layer-colorpicker-main-pre">'];
          layui.each(options.colors, function(i, v){
            list.push(['<div class="layer-colorpicker-pre'+ ((v.match(/[0-9]{1,3}/g) || []).length > 3
              ? ' layer-colorpicker-pre-isalpha'
            : '') +'">'
      ...
      //底部表单元素区域
      ,'<div class="layer-colorpicker-main-input">'
        ,'<div class="layer-inline">'
          ,'<input type="text" class="layer-input">'
        ,'</div>'
        ,'<div class="layer-btn-container">'
          ,'<button class="layer-btn layer-btn-primary layer-btn-sm" colorpicker-events="clear">清空</button>'
          ,'<button class="layer-btn layer-btn-sm" colorpicker-events="confirm">确定</button>'
        ,'</div'
      ,'</div>'
    ,'</div>'].join(''))
  ...
  Class.prototype.removePicker = function(index){
    var that = this
    ,options = that.config;
    $('#layer-colorpicker'+ (index || that.index)).remove();
    return that;
  };
  ...
  Class.prototype.side = function(){
    ...
    ,pre = that.elemPicker.find('.layer-colorpicker-pre').children('div')
    ...
    //拖拽元素
    ,elemMove = $(['<div class="layer-auxiliar-moving" id="LAY-colorpicker-moving"></div'].join(''))
    ...
    pre.each(function(){
      $(this).on('click', function(){
        $(this).parent('.layer-colorpicker-pre').addClass('selected').siblings().removeClass('selected');
