...
layui.define('form', function(exports){
  ...
  //字符常量
  ,SHOW = 'layer-show', HIDE = 'layer-hide', NONE = 'layer-none', DISABLED = 'layer-disabled'
  
  ,ELEM_VIEW = 'layer-tree', ELEM_SET = 'layer-tree-set', ICON_CLICK = 'layer-tree-iconClick'
  ,ICON_ADD = 'layer-icon-addition', ICON_SUB = 'layer-icon-subtraction', ELEM_ENTRY = 'layer-tree-entry', ELEM_MAIN = 'layer-tree-main', ELEM_TEXT = 'layer-tree-txt', ELEM_PACK = 'layer-tree-pack', ELEM_SPREAD = 'layer-tree-spread'
  ,ELEM_LINE_SHORT = 'layer-tree-setLineShort', ELEM_SHOW = 'layer-tree-showLine', ELEM_EXTEND = 'layer-tree-lineExtend'
  ...
  Class.prototype.render = function(){
    ...
    var temp = $('<div class="layer-tree'+ (options.showCheckbox ? " layer-form" : "") + (options.showLine ? " layer-tree-line" : "") +'" lay-filter="LAY-tree-'+ that.index +'"></div>');
    that.tree(temp);
    ...
    //插入组件结构
    that.elem = temp;
    that.elemNone = $('<div class="layer-tree-emptyText">'+ options.text.none +'</div>');
    othis.html(that.elem);

    if(that.elem.find('.layer-tree-set').length == 0){
    ...
    that.elem.find('.layer-tree-set').each(function(){
      var othis = $(this);
      //最外层
      if(!othis.parent('.layer-tree-pack')[0]){
        othis.addClass('layer-tree-setHide');
      };

      //没有下一个节点 上一层父级有延伸线
      if(!othis.next()[0] && othis.parents('.layer-tree-pack').eq(1).hasClass('layer-tree-lineExtend')){
        othis.addClass(ELEM_LINE_SHORT);
      };
      
      //没有下一个节点 外层最后一个
      if(!othis.next()[0] && !othis.parents('.layer-tree-set').eq(0).next()[0]){
        othis.addClass(ELEM_LINE_SHORT);
      };
    });
  ...
  Class.prototype.tree = function(elem, children){
    ...
    layui.each(data, function(index, item){
      ...
      ,packDiv = $('<div class="layer-tree-pack" '+ (item.spread ? 'style="display: block;"' : '') +'"></div>')
      ,entryDiv = $(['<div data-id="'+ item.id +'" class="layer-tree-set'+ (item.spread ? " layer-tree-spread" : "") + (item.checked ? " layer-tree-checkedFirst" : "") +'">'
        ,'<div class="layer-tree-entry">'
          ,'<div class="layer-tree-main">'
            //箭头
            ,function(){
              if(options.showLine){
                if(hasChild){
                  return '<span class="layer-tree-iconClick layer-tree-icon"><i class="layer-icon '+ (item.spread ? "layer-icon-subtraction" : "layer-icon-addition") +'"></i></span>';
                }else{
                  return '<span class="layer-tree-iconClick"><i class="layer-icon layer-icon-file"></i></span>';
                };
              }else{
                return '<span class="layer-tree-iconClick"><i class="layer-tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>';
              };
            }()
      ...
      ,function(){
        ...
        var editIcon = {
          add: '<i class="layer-icon layer-icon-add-1"  data-type="add"></i>'
          ,update: '<i class="layer-icon layer-icon-edit" data-type="update"></i>'
          ,del: '<i class="layer-icon layer-icon-delete" data-type="del"></i>'
        }, arr = ['<div class="layer-btn-group layer-tree-btnGroup">'];
      ...
      if(entryDiv.prev('.'+ELEM_SET)[0]){
        entryDiv.prev().children('.layer-tree-pack').addClass('layer-tree-showLine');
      };
      
      //若无子节点，则父节点加延伸线
      if(!hasChild){
        entryDiv.parent('.layer-tree-pack').addClass('layer-tree-lineExtend');
      };
  ...
  Class.prototype.spread = function(elem, item){
    ...
    touchOpen.on('click', function(e){
      ...
      ,iconClick = touchOpen.children('.layer-icon')[0] ? touchOpen.children('.layer-icon') : touchOpen.find('.layer-tree-icon').children('.layer-icon');
      ...
      if(!packCont[0]){
        ...
        if(elem.hasClass(ELEM_SPREAD)){
          ...
          if(options.accordion){
            ...
            sibls.find('.layer-tree-icon').children('.layer-icon').removeClass(ICON_SUB).addClass(ICON_ADD);
          };
        };
      };
    });
  ...
  Class.prototype.operate = function(elem, item){
    ...
    entry.children('.layer-tree-btnGroup').on('click', '.layer-icon', function(e){
      ...
      if(type == 'add'){
        ...
        if(!packCont[0]){
          //若开启连接线，更改图标样式
          if(options.showLine){
            elemMain.find('.'+ICON_CLICK).addClass('layer-tree-icon');
            elemMain.find('.'+ICON_CLICK).children('.layer-icon').addClass(ICON_ADD).removeClass('layer-icon-file');
          //若未开启连接线，显示箭头
          }else{
            elemMain.find('.layer-tree-iconArrow').removeClass(HIDE);
          };
          //节点添加子节点容器
          elem.append('<div class="layer-tree-pack"></div>');
        };
      ...
      //修改
        ...
        //添加输入框，覆盖在文字上方
        elemMain.append('<input type="text" class="layer-tree-editInput">');
        //获取焦点
        elemMain.children('.layer-tree-editInput').val(text).focus();
        //嵌入文字移除输入框
        ...
        //失去焦点
        elemMain.children('.layer-tree-editInput').blur(function(){
          getVal($(this));
        });
        //回车
        elemMain.children('.layer-tree-editInput').on('keydown', function(e){
          if(e.keyCode === 13){
            e.preventDefault();
            getVal($(this));
          };
      ...
      //删除
        ...
        layer.confirm('确认删除该节点 "<span style="color: #999;">'+ (item.title || '') +'</span>" 吗？', function(index){
          ...
          if(elem.siblings('.'+ELEM_SET).children('.'+ELEM_ENTRY)[0]){
            ...
            //若开启了连接线
            if(options.showLine){
              prevDiv.find('.'+ICON_CLICK).removeClass('layer-tree-icon');
              prevDiv.find('.'+ICON_CLICK).children('.layer-icon').removeClass(ICON_SUB).addClass('layer-icon-file');
              //父节点所在层添加延伸线
            ...
            //父节点隐藏箭头
              prevDiv.find('.layer-tree-iconArrow').addClass(HIDE);
            };
            //移除展开属性
  ...
  Class.prototype.events = function(){
    var that = this
    ,options = that.config
    ,checkWarp = that.elem.find('.layer-tree-checkedFirst');
    
    //初始选中
    that.setChecked(that.checkids);
    
    //搜索
    that.elem.find('.layer-tree-search').on('keyup', function(){
      var input = $(this)
      ,val = input.val()
      ,pack = input.nextAll()
      ,arr = [];
      ...
      pack.find('.'+ ELEM_TEXT).each(function(){
        ...
        if($(this).html().indexOf(val) != -1){
          ...
          var select = function(div){
            div.addClass('layer-tree-searchShow');
            //向上父节点渲染
      ...
      pack.find('.'+ELEM_ENTRY).each(function(){
        ...
        if(!parent.hasClass('layer-tree-searchShow')){
          parent.addClass(HIDE);
        };
      });
      if(pack.find('.layer-tree-searchShow').length == 0){
        that.elem.append(that.elemNone);
      };
    ...
    //还原搜索初始状态
    that.elem.find('.layer-tree-search').on('keydown', function(){
      $(this).nextAll().find('.'+ELEM_ENTRY).each(function(){
        var parent = $(this).parent('.'+ELEM_SET);
        parent.removeClass('layer-tree-searchShow '+ HIDE);
      });
      if($('.layer-tree-emptyText')[0]) $('.layer-tree-emptyText').remove();
    });
  };
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点找到选中索引
    that.elem.find('.layer-form-checked').each(function(){
