...
layui.define(['laytpl', 'form'], function(exports){
  ...
  //字符常量
  ,ELEM = 'layer-transfer', HIDE = 'layer-hide', DISABLED = 'layer-btn-disabled', NONE = 'layer-none'
  ,ELEM_BOX = 'layer-transfer-box', ELEM_HEADER = 'layer-transfer-header', ELEM_SEARCH = 'layer-transfer-search', ELEM_ACTIVE = 'layer-transfer-active', ELEM_DATA = 'layer-transfer-data'
  
  //穿梭框模板
  ,TPL_BOX = function(obj){
    obj = obj || {};
    return ['<div class="layer-transfer-box" data-index="'+ obj.index +'">'
      ...
      ,'<div class="layer-transfer-search">'
        ,'<i class="layer-icon layer-icon-search"></i>'
        ,'<input type="input" class="layer-input" placeholder="关键词搜索">'
      ,'</div>'
      ,'{{# } }}'
      ,'<ul class="layer-transfer-data"></ul>'
    ,'</div>'].join('');
  }
  
  //主模板
  ,TPL_MAIN = ['<div class="layer-transfer layer-form layer-border-box" lay-filter="LAY-transfer-{{ d.index }}">'
    ,TPL_BOX({
      index: 0
      ,checkAllName: 'layTransferLeftCheckAll'
    })
    ,'<div class="layer-transfer-active">'
      ,'<button type="button" class="layer-btn layer-btn-sm layer-btn-primary layer-btn-disabled" data-index="0">'
        ,'<i class="layer-icon layer-icon-next"></i>'
      ,'</button>'
      ,'<button type="button" class="layer-btn layer-btn-sm layer-btn-primary layer-btn-disabled" data-index="1">'
        ,'<i class="layer-icon layer-icon-prev"></i>'
      ,'</button>'
    ,'</div>'
    ,TPL_BOX({
      index: 1
  ...
  Class.prototype.render = function(){
    ...
    that.layBtn = thisElem.find('.'+ ELEM_ACTIVE + ' .layer-btn');
  ...
  Class.prototype.noneView = function(thisDataElem, text){
    var createNoneElem = $('<p class="layer-none">'+ (text || '') +'</p>');
