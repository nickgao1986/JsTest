...
;!function(win){
  ...
  Layui.prototype.define = function(deps, factory){
    ...
    if((!layui['layui.all'] && layui['layer.mobile'])){
  ...
  Layui.prototype.use = function(apps, callback, exports){
    ...
    //如果引入了完整库（layer.all.js），内置的模块则不必再加载
    if(apps.length === 0 
    || (layui['layui.all'] && modules[item]) 
    || (!layui['layui.all'] && layui['layer.mobile'] && modules[item])
    ){
